---
layout: default
title: Manifeste Complet
description: Les sept principes éthiques, les trois contrats néguentropiques, et la transition vers une politique qui apprend
---
...
```mermaid
graph LR
    A[Systèmes Politiques] -->|Entropie ⬇️| B[Désordre, Désinformation, Cynisme]
    A-->|Néguentropie ⬆️| C[Transparence, Apprentissage, Coopération]
    style B fill:#ffcccc
    style C fill:#ccffcc
```
...
```mermaid
graph TD
    A[Réel en mutation] -->|1. Détection| B[Dérive sociétale]
    B -->|2. Capture| C[Vérité Terrain]
    C -->|3. Apprentissage| D[Adaptation des politiques]
    D -->|Renforcement| A
    style D fill:#ccffcc
```
